...
/**
 * Sincronización masiva: busca en CARRERA_ADMINISTRATIVA todos los empleados
 * cuyo DNI (DOCUMENTO_EMPLEADO) NO existe todavía en DATOS_PERSONALES_AGENTE_JUBILA,
 * y los agrega con sus datos de carrera.
 *
 * Retorna: { ok, added, skipped, errors, lastUpdated }
 */
export async function bulkSyncAgentes(): Promise<{
  ok: boolean
  added: number
  skipped: number
  errors: number
  lastUpdated: string
  newAgents: { dni: string; nombre: string }[]
  error?: string
}> {
  try {
    // 1. Obtener todos los DNIs ya registrados en datos personales
    const existentes = await prisma.dATOS_PERSONALES_AGENTE_JUBILA.findMany({
      select: { DNI_AGENTE: true },
    })
    const dniSet = new Set(existentes.map((e) => e.DNI_AGENTE))

    // 2. Obtener todos los registros de CARRERA_ADMINISTRATIVA con DNI único
    const carreras = await prisma.cARRERA_ADMINISTRATIVA.findMany({
      select: {
    ...
    // Agrupar por DNI para obtener la fecha alta más antigua (primer ingreso)
    const carrerasPorDni = new Map<string, { fechaAlta: Date | null }>()
    for (const c of carreras) {
      const dni = c.DOCUMENTO_EMPLEADO.trim()
      if (!carrerasPorDni.has(dni)) {
        carrerasPorDni.set(dni, { fechaAlta: c.FECHA_ALTA })
      }
    }

    // 3. Filtrar los DNI que no están en el sistema
    const nuevos = [...carrerasPorDni.entries()].filter(([dni]) => !dniSet.has(dni))

    let added = 0
    let errors = 0
    const newAgents: { dni: string; nombre: string }[] = []

    // 4. Crear los agentes faltantes
    for (const [dni] of nuevos) {
      try {
        // Verificar que el DNI no haya sido creado en una iteración anterior (race condition)
        const yaExiste = await prisma.dATOS_PERSONALES_AGENTE_JUBILA.findUnique({
          where: { DNI_AGENTE: dni },
        })
        if (yaExiste) {
          continue
        ...
        const nuevo = await prisma.dATOS_PERSONALES_AGENTE_JUBILA.create({
          data: {
            DNI_AGENTE: dni,
            NOMBRE_AGENTE: 'Sin datos',
            APELLIDO_AGENTE: 'Sin datos',
            FECHA_NACIMIENTO: new Date(1970, 0, 1),
            ESTADO_ACTIVO: true,
            ID_REGIMEN_JUBILATORIO: 1,
          },
        })
        newAgents.push({ dni, nombre: `${nuevo.APELLIDO_AGENTE} ${nuevo.NOMBRE_AGENTE}` })
        added++
      } catch (err) {
        console.error(`[bulkSyncAgentes] Error al crear agente DNI ${dni}:`, err)
        errors++
      }
    }
    ...
    const lastUpdated = `${dd}/${mm}/${yyyy} ${hh}:${min}`
    ...
    console.error('[bulkSyncAgentes] Error:', error)
    return {
      ok: false,
      added: 0,
      skipped: 0,
